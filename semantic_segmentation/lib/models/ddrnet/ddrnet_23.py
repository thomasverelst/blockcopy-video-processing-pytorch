...
BatchNorm2d = nn.BatchNorm2d #nn.SyncBatchNorm
bn_mom = 0.1
...
class DualResNet(nn.Module):
    ...
    def load_state_dict(self, state_dict: 'OrderedDict[str, Tensor]', strict: bool):
        ...
        for k,v in state_dict.items():
            ...
            new_state_dict[k] = v
        if strict:
            warnings.warn('Load_state_dict received strict = True but will use strict=False')
    ...
    def forward(self, x):
        ...
        with timings.env('model/layer1',4):
            x = self.conv1(x)

            x = self.layer1(x)
            layers.append(x)

        with timings.env('model/layer2',4):
        ...
        with timings.env('model/layer4+',4):
            x = self.layer4(self.relu(x))
            layers.append(x)
            x_ = self.layer4_(self.relu(x_))

        with timings.env('model/layer4+shift',4):
        ...
        with timings.env('model/layer5+',4):
        ...
        with timings.env('model/final_layer',4):
