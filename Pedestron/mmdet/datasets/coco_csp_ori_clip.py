...
class CocoCSPORIClipDataset(CustomDataset):
    ...
    def prepare_train_img(self, idx):
        ...
        # img = mmcv.imread(osp.join(self.img_prefix, fn.replace('.png', '.jpg')))
        img = mmcv.imread(osp.join(self.img_prefix, fn))
    ...
    def prepare_test_img(self, idx):
        ...
        assert len(self.img_scales) == 1, "only support single scale evaluation"
        assert self.flip_ratio == 0, "clip dataloader does not support flip"

        SKIP_AMOUNT = 1
        fn = img_info['filename']
        fn = fn.replace('_leftImg8bit.png', '')
        fn_parts = fn.split('_')
        fn_prefix = '_'.join(fn_parts[:-1])+'_'
        fn_frame_id = int(fn_parts[-1])
        fn_suffix = '_leftImg8bit.png'
        
        for j in range(0, self.clip_length, SKIP_AMOUNT):
            this_frame_id = fn_frame_id - j
            this_fn = fn_prefix + str(this_frame_id).zfill(6) + fn_suffix
            
            img = mmcv.imread(osp.join(self.img_prefix,this_fn))
            cimg, _img_meta, _proposal = prepare_single(img, self.img_scales[0], False, proposal)
            imgs.append(cimg)
            _img_meta['filename'] = this_fn
            img_metas.append(DC(_img_meta, cpu_only=True))
            # img_metas.append(img_metas[0])
        imgs = list(reversed(imgs))
        img_metas = list(reversed(img_metas))
