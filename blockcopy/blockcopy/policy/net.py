...
class PolicyNet(nn.Module):
    def __init__(self, settings) -> None:
        ...
        mid_channels = self.backbone.OUT_CHANNELS
        
        strides = [2,2,2]
        head = []
        head.append(self._make_layer(mid_channels, mid_channels, kernel_size=self.kernel_size, stride=strides[0], bnrelu=True))
        head.append(self._make_layer(mid_channels, mid_channels, kernel_size=self.kernel_size, stride=strides[1], bnrelu=True))
    ...
    def forward(self, policy_meta: Dict):
        frame = policy_meta['inputs'].float()
        N,C,H,W = frame.shape
        ...
        input_features.append(F.interpolate(frame, scale_factor=self.scale_factor, mode='nearest'))
        
        if self.use_frame_state:
            # frame state (last executed frame per block)
            frame_state = policy_meta['frame_state'].float()
            input_features.append(F.interpolate(frame_state, size=input_features[0].shape[2:], mode='nearest'))
            
        if self.use_prev_output:
            assert policy_meta.get('output_repr', None) is not None
            outputs = policy_meta['output_repr'].type(frame.dtype)
