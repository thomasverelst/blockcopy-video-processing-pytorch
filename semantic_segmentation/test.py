...
def main():
    ...
    parser.add_argument("--network", default="swiftnet_resnet18", type=str, help="")
    ...
    ## Model
    if args.network.startswith('swiftnet_'):
        backbone = args.network.split('_')[-1]
        backbone = models.swiftnet.backbones.resnet.__dict__[backbone](pretrained=False)
        model = models.swiftnet.swiftnet.SwiftNet(backbone=backbone, num_classes=num_classes, num_features=128, use_spp=True)
    if args.network.startswith('ddrnet_'):
        model = models.ddrnet.ddrnet_39.get_seg_model({})
    ...
    def process_dataset(dataloader, phase, max_num_clips=-1):
        ...
        assert total_num_clips > 1, "must be more than 1 clip"
        ...
        for iteration in tqdm.tqdm(range(total_num_clips)):
            ...
            num_timing_images += len(clip)*clip[0].size(0)
        ...
        if phase == 'eval':
            ...
            if args.fast:
                logging.info(f'Average FPS: {num_timing_images/(stop - start):.2f} over {num_timing_images}')
            if count_flops:
