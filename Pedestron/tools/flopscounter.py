...
def compute_total_flops_cost(self, submodule_depth=0):
    """
    A method that will be available after add_flops_counting_methods() is called
    on a desired net object.

    Returns total flops consumption of all images.
    ...
    # build module list
    modules_at_depth = defaultdict(list, {0: [('model', self),]})
    ...
    # print(modules_flops)
    return modules_flops[0]['model'][0], modules_flops, num_images

def total_flops_cost_repr(self, submodule_depth=0, units='GMac', precision=2):
    _, modules_flops, batches_count = self.compute_total_flops_cost(submodule_depth)
    s = '======= FLOPSCOUNTER =======\n'
    s += f'batches: {batches_count}\n'
...
def conv_flops_counter_hook(conv_module, input, output):
    ...
    if hasattr(conv_module, '__mask__') and conv_module.__mask__ is not None:
        assert False
        active_elements_count = conv_module.__mask__.hard.sum().item()
    else:
    ...
    # print('FLOPS PER POSITION', conv_per_position_flops, active_elements_count)
    # print('FLOPS', overall_flops)
...
def add_flops_counter_hook_function(module):
    if is_supported_instance(module):
        if hasattr(module, '__flops_handle__'):
            return
        function = MODULES_MAPPING[type(module)]
